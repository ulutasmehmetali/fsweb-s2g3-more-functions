...
function ortalamaBul(sayilar) {
  ...
  const toplam = sayilar.reduce((previous, current) => current += previous);
...
function buyukEsitOrtalama(arr, ortalamaBulFn) {
  return arr.filter((num) => num >= ortalamaBulFn(arr));
}
...
console.log(buyukEsitOrtalama(array, ortalamadanBuyukleriBul));
